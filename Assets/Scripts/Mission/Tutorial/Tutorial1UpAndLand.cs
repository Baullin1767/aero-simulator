...
namespace Mission.Tutorial
{
    ...
    public class HeightMissionStep : MissionStep
    {
        ...
        public override void Initialize()
        {
            ...
            if (_timer == null)
            {
                _timer = Object.FindObjectOfType<Timer>(true);
        ...
        {
            _timer.gameObject.SetActive(heightTimeSeconds > 1.1f);
            var time = 0f;
            while (time < heightTimeSeconds)
            {
                ...
                var lastTime = heightTimeSeconds - time;
                _timer.SetText(lastTime.ToString("ОСТАЛОСЬ НА ВЫСОТЕ: 0:00"));
            }

            if (heightTimeSeconds > 0.5f) _timer.gameObject.SetActive(false);
        }
    }
