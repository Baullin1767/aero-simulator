...
namespace YueUltimateDronePhysics
{
    ...
    public class YueDronePhysics : MonoBehaviour
    {
        ...
        public Transform TargetQuad => targetQuad;
        ...
        void Start()
        {
            ...
            targetQuad.SetLocalPositionAndRotation(transform.localPosition, transform.localRotation);
        ...
        private void FixedUpdate()
        {
            ...
            if (armed)
            {
                ...
                if (rb.velocity.sqrMagnitude < 0.0005) targetAltitude = transform.position.y;//ResetInternals();
        ...
        private void AcroMode()
        {
            if (highSpeed)
            {
                transform.localRotation *= Quaternion.Euler(inputModule.pitch * Time.deltaTime, inputModule.yaw * Time.deltaTime, inputModule.roll * Time.deltaTime);
                rb.angularVelocity = new Vector3(inputModule.pitch * Time.deltaTime * rb.angularVelocity.x, 
                    inputModule.yaw * Time.deltaTime * rb.angularVelocity.y, 
                    inputModule.roll * Time.deltaTime * rb.angularVelocity.z);
            }
            else
            {
        ...
        private void AltitudeHold()
        {
            ...
            pdForce = altitudePID.CalculatePD(error * Vector3.one, physicsConfig.pAltitudeR, physicsConfig.dAltitudeR,
                physicsConfig.pAltitudeP, physicsConfig.dAltitudeP,
                physicsConfig.pAltitudeY, physicsConfig.dAltitudeY).y;
        ...
        private Vector3 CalculatePIDTorque()
        {
            ...
            //Clip errorMagnitude
            if (errorMagnitude >= 120 || errorMagnitude <= -120)
            {
                highSpeed = true;
        ...
        public void ExtraLandingMode(bool mode)
        {
            rb.isKinematic = mode;
        }
    }
